...
from mynlplib import preproc, clf_base, constants, hand_weights, evaluation, naive_bayes, perceptron, logreg, features
import numpy as np
import torch
...
def test_d6_1_topfeat():
    top_feats_two = features.get_top_features_for_label(hand_weights.theta_hand,'2000s',3)
    eq_(top_feats_two[0],(('2000s', 'name'), 0.2))
    eq_(len(top_feats_two),3)
    
    top_feats_eighty = features.get_top_features_for_label(hand_weights.theta_hand,'1980s',3)
